...
const calculateCpfCheckDigit = (baseDigits: string, factorStart: number): number => {
  let total = 0;
...
const calculateCnpjCheckDigit = (baseDigits: string, factors: number[]): number => {
  let total = 0;
  for (let index = 0; index < baseDigits.length; index += 1) {
    total += Number(baseDigits[index]) * (factors[index] ?? 0);
  }
  const remainder = total % 11;
  return remainder < 2 ? 0 : 11 - remainder;
};

export const normalizeDocumentDigits = (value: unknown): string => {
...
export const normalizeCpfCnpj = (value: unknown): string | null => {
  const digits = normalizeDocumentDigits(value);

/**
 * messagesUpsertHandler.test.js — Integration tests for handleMessagesUpsert.
 *
 * Mocks: pool (DB), logger, botOrchestrator.
 * Uses real phoneUtils + payloadParser + deduplication logic.
 */
...
const mockBotProcess = jest.fn().mockResolvedValue({ ok: true, responseSent: true });
const mockLogger = {
  info: jest.fn(), warn: jest.fn(), debug: jest.fn(), error: jest.fn(),
};

// Mock resolveTenant directly — avoids tenantCache singleton bleeding between tests
jest.unstable_mockModule('../services/whatsapp/tenant/tenantResolver.js', () => ({
...
jest.unstable_mockModule('../bot/botOrchestrator.js', () => ({
  process: mockBotProcess,
  initBotOrchestrator: jest.fn(),
}));
...
const BOT_NUMBER = '5583996311811';        // 13-digit E.164 (canonical: 8396311811)
const CLIENT_NUMBER = '558396311811';      // 12-digit E.164 (canonical: 558396311811 ≠ bot)
...
describe('handleMessagesUpsert', () => {
  ...
  describe('Passo 6 — self-message check (o único no sistema)', () => {
    ...
    // Critical regression: 12d client ≠ 13d bot → must NOT be blocked
    ...
    it('does NOT block author 83996311811 (11d) when bot is 5583996311811 (13d) — same person', async () => {
      // These canonicalize to the same → should block
      const payload = {
        event: 'messages-upsert',
        instance: INSTANCE,
        data: {
  ...
  describe('Passo 9-10 — fluxo completo (happy path)', () => {
    ...
    it('connectedNumber nunca logado completo (apenas últimos 4)', () => {
      // Verify no info/warn call contains the full connectedNumber

...
const mockAsaasClient = vi.hoisted(() => ({
  get: vi.fn(),
  post: vi.fn(),
  put: vi.fn(),
  delete: vi.fn(),
...
vi.mock('../integrations/asaas/client.js', () => ({
  asaasClient: mockAsaasClient,
  normalizeAsaasApiKey: (value: unknown) => String(value || '').trim().replace(/^Bearer\s+/i, ''),
  removeEmptyFields: (payload: Record<string, unknown>) =>
    Object.fromEntries(
...
describe('asaasService', () => {
  ...
  it('reuses customer already saved in database without calling Asaas', async () => {
    const onCustomerResolved = vi.fn();

...
const mockGetInstanceMe = vi.hoisted(() => vi.fn());
const mockCreateInstance = vi.hoisted(() => vi.fn());
const mockConnectInstance = vi.hoisted(() => vi.fn());
const mockGetQrCode = vi.hoisted(() => vi.fn());
const mockLogoutInstance = vi.hoisted(() => vi.fn());
const mockGetEvolutionConfig = vi.hoisted(() => vi.fn());
const mockIsSessionStateError = vi.hoisted(() => vi.fn());
const mockGetProviderErrorMessage = vi.hoisted(() => vi.fn());
const mockIsInstanceNotFoundError = vi.hoisted(() => vi.fn());
const mockEnsureBarbershopWhatsAppInstanceName = vi.hoisted(() => vi.fn());
const mockGetBarbershopWhatsAppInstanceContext = vi.hoisted(() => vi.fn());
const mockNormalizeWhatsAppInstanceName = vi.hoisted(() => vi.fn());
...
vi.mock('../services/evolutionApiService.js', () => ({
...
describe('whatsappClient destination resolution', () => {
  beforeEach(() => {
    ...
    mockNormalizeWhatsAppInstanceName.mockImplementation((value: unknown) =>
  ...
  it('sets status as instance_not_found when Evolution reports missing instance', async () => {
    ...
    mockIsInstanceNotFoundError.mockImplementation((error: unknown) => Number((error as { status?: number })?.status || 0) === 404);

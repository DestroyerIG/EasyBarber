...
describe('parseEvolutionWebhook', () => {
  ...
  describe('payloads inválidos', () => {
    ...
    it('retorna null para string', () => expect(parseEvolutionWebhook('texto' as unknown as Record<string, unknown>)).toBeNull());
    it('retorna null sem event', () => {
      expect(parseEvolutionWebhook({ instance: 'x', data: {} } as unknown as Record<string, unknown>)).toBeNull();
    });
    it('retorna null sem instanceName', () => {
      expect(parseEvolutionWebhook({ event: 'messages-upsert', data: {} } as unknown as Record<string, unknown>)).toBeNull();
    });
    it('nunca lança exceção', () => {
      expect(() => parseEvolutionWebhook(undefined as unknown as Record<string, unknown>)).not.toThrow();
      expect(() => parseEvolutionWebhook(42 as unknown as Record<string, unknown>)).not.toThrow();
      expect(() => parseEvolutionWebhook({ event: null } as unknown as Record<string, unknown>)).not.toThrow();
    });
  });
});

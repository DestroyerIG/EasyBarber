...
-- Run this on existing databases to upgrade from v1

BEGIN;

-- 1. Add CHECK constraints to existing columns
-- (safe: ALTER TABLE ... ADD CONSTRAINT ... CHECK ... NOT VALID then VALIDATE)

ALTER TABLE barbershops DROP CONSTRAINT IF EXISTS chk_barbershops_plan;
ALTER TABLE barbershops ADD CONSTRAINT chk_barbershops_plan CHECK (plan IN ('basico', 'profissional', 'premium'));

ALTER TABLE users DROP CONSTRAINT IF EXISTS chk_users_role;
ALTER TABLE users ADD CONSTRAINT chk_users_role CHECK (role IN ('admin', 'barber'));

ALTER TABLE appointments DROP CONSTRAINT IF EXISTS chk_appointments_status;
ALTER TABLE appointments ADD CONSTRAINT chk_appointments_status CHECK (status IN ('confirmado', 'concluido', 'cancelado'));
...
ALTER TABLE services ADD CONSTRAINT chk_services_duration CHECK (duration_minutes >= 10 AND duration_minutes <= 300);

ALTER TABLE earnings DROP CONSTRAINT IF EXISTS chk_earnings_amount;
ALTER TABLE earnings ADD CONSTRAINT chk_earnings_amount CHECK (amount > 0);
...
-- May fail if duplicates exist - clean up first if needed
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'clients_barbershop_id_phone_key'
    ) THEN
        ALTER TABLE clients ADD CONSTRAINT clients_barbershop_id_phone_key UNIQUE (barbershop_id, phone);
    END IF;
END;
$$;
...
DECLARE
    t TEXT;
BEGIN
    FOR t IN SELECT unnest(ARRAY['barbershops','users','barbers','services','clients','appointments','expenses'])
    LOOP
        EXECUTE format('

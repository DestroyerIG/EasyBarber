/**
 * phoneUtils.ts — Normalização e comparação de telefones para WhatsApp / Evolution API.
 *
 ...
 */
export const stripToDigits = (raw: string): string => {
 ...
 */
export const canonicalizePhone = (raw: string): string => {
 ...
 */
export const isSelfMessage = (authorRaw: string, connectedRaw: string): boolean => {
 ...
 */
export const resolveAuthorPhone = (payload: unknown): ResolvedAuthor => {
  const p = payload as Record<string, unknown> | null;
  const data = (p?.data ?? p) as Record<string, unknown> | null;
  const key = (data?.key ?? (p?.key) ?? {}) as Record<string, unknown>;
